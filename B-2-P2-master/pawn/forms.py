...
from .exceptions import PawnImageUploadError

from .settings import (
    PAWN_IMAGE_MAX_SIZE,
    PAWN_MEDIA_PATH,
    PAWN_UPLOAD_CONTENT_TYPES,
    PAWN_UPLOAD_MAX_SIZE,
    PAWN_SVG_JAVASCRIPT_PROTECTION
)
...
class ImageForm(forms.Form):
    ...
    def save(self, commit=True):
        ...
        *Note*: The dimension of image files (excluding SVG) are set using ``PIL``.
        ...
        if (content_type.lower() == self._SVG_TYPE
                and PAWN_SVG_JAVASCRIPT_PROTECTION
                and xml_has_javascript(uploaded_image.read())):

            raise PawnImageUploadError(
    ...
    def _save(self, image, file_name, commit):
        ...
        full_path = path.join(PAWN_MEDIA_PATH, unique_file_name)
    ...
    def _process_raster(image, extension):
        ...
        *Note*: The file needs to be uploaded and saved temporarily in the
        ...
        """
        thumb_io = BytesIO()
        preped_image = scale_and_crop(image, **PAWN_IMAGE_MAX_SIZE)
    ...
    def clean(self):
        ...
        # -----------------------------------------------
        if not upload:
            raise PawnImageUploadError.not_uploaded()

        content_type = upload.content_type
        file_size = upload.size

        if content_type not in PAWN_UPLOAD_CONTENT_TYPES:

            raise PawnImageUploadError.unsupported_format()

        elif file_size > PAWN_UPLOAD_MAX_SIZE:

            raise PawnImageUploadError.invalid_size(
                current=file_size,
                expected=PAWN_UPLOAD_MAX_SIZE
            )
